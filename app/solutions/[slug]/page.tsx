    ...
    <ul className="space-y-4">
      {items.map((item) => (
        <li key={item} className="flex gap-4 text-[15px] leading-relaxed text-mist">
          <span className="mt-[0.65rem] h-px w-5 shrink-0 bg-ember/60" aria-hidden="true" />
          <span>{item}</span>
        </li>
      ))}
    </ul>
      ...
      <header className="relative overflow-hidden pb-16 pt-28 md:pt-32">
        ...
        <div className="wrap relative">
          <Reveal>
            <Link
              href="/#solutions"
              className="focus-ring rounded-sm font-label text-[13px] font-semibold text-mist hover:text-paper"
            >
              ← All solutions
            </Link>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <p className="eyebrow">{s.category}</p>
              <span className="rounded-full border border-seam px-3 py-1 font-label text-[10px] font-semibold uppercase tracking-wider text-mist">
                {s.status}
              </span>
            </div>
            ...
            <dl className="mt-10 grid max-w-2xl grid-cols-2 gap-6 border-t border-seam pt-6 sm:grid-cols-3">
              <div>
              ...
              <div>
                <dt className="font-label text-[11px] font-semibold uppercase tracking-caps text-mist">Role</dt>
                <dd className="mt-1 font-label text-sm font-semibold text-paper">End-to-end owner</dd>
              </div>
              <div>
                <dt className="font-label text-[11px] font-semibold uppercase tracking-caps text-mist">Focus</dt>
                <dd className="mt-1 font-label text-sm font-semibold text-paper">{s.focus[0]}</dd>
              </div>
            </dl>
          </Reveal>
        </div>
      ...
      <div className="wrap pb-24">
        <Block label="Overview">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {s.focus.map((f) => (
              <span key={f} className="rounded-md bg-charcoal px-3 py-1.5 font-label text-[12px] font-semibold text-mist">
                {f}
              </span>
            ))}
          </div>
        ...
        <Block label="Solution architecture">
          ...
          <div className="mt-10">
            ...
            <CaseImage
              src={`/case-studies/architecture-${s.slug}.png`}
              alt={`${s.name} architecture diagram`}
              title="Architecture diagram"
              note="Add /public/case-studies/architecture-{slug}.png to replace this placeholder."
            />
          </div>
        </Block>
        ...
        <Block label="Technology stack">
          <ul className="flex flex-wrap gap-2">
            {s.stack.map((t) => (
              <li key={t} className="rounded-full border border-seam bg-charcoal px-4 py-1.5 font-label text-[13px] font-semibold text-paper/90">
                {t}
              </li>
            ))}
          </ul>
        ...
        <Block label="Implementation journey">
          ...
          <div className="mt-10">
            {/* Implementation journey block */}
            <CaseImage
              src={`/case-studies/workflow-${s.slug}.png`}
              alt={`${s.name} workflow`}
              title="Workflow visualization"
              note="Add /public/case-studies/workflow-{slug}.png to replace this placeholder."
            />
          </div>
        </Block>
        ...
        <Block label="Future improvements">
          <List items={s.future} />
          <div className="mt-10">
            {/* Future improvements block */}
            <CaseImage
              src={`/case-studies/screenshot-${s.slug}.png`}
              alt={`${s.name} screenshot`}
              title="Screenshots"
              note="Add /public/case-studies/screenshot-{slug}.png to replace this placeholder."
            />
          </div>
        </Block>

        <Reveal>
          <div className="mt-4 flex flex-col items-start justify-between gap-6 rounded-2xl border border-seam bg-graphite p-8 md:flex-row md:items-center">
            <div>
            ...
            <Link
              ...
              className="focus-ring rounded-full bg-ember px-6 py-3 font-label text-sm font-bold text-ink transition-transform hover:-translate-y-0.5"
            >
              Read next →
            </Link>
          </div>
